...
var modelHelper = module.exports = {
  ...
  getPropType: function getPropType(propType) {
    ...
    } else if (propType == 'array' || Array.isArray(propType)) {
      return 'array';
  ...
  LDLPropToSwaggerDataType: function LDLPropToSwaggerDataType(ldlType) {
    ...
    //console.log('after pick, for ldlType = %s, swaggerType = %s', JSON.stringify(ldlType), JSON.stringify( swaggerType));
    ...
    if (swaggerType.type === 'array') {
      ...
      if (arrayItem) {
        ...
        var items = ldlType.items || {};
        if(items.type) {
          swaggerType.items = { type: items.type };
        }
        else {
